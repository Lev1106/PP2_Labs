...
# For convenience (+ functions from previous labs)
...
def query_pagination(order_field="surname", limit=None, offset=None):
	conn = connect()
	cursor = conn.cursor()

	q = f"""
		SELECT users.id, users.name, users.surname, phonebook.phone
		FROM users JOIN phonebook ON users.id = phonebook.user_id
		ORDER BY {order_field}
	"""
	
	# to correctly work if limit and/or offset are not defined
	if limit:
		q += (f" LIMIT {limit}")
	if offset:
		q += (f" OFFSET {offset}")
...
if __name__ == "__main__":
	...
	print("=" * 50)
	print("Task 1. Universal select")
	print(query_data(output=True))
	print(query_data("A", "B", "+77777777777", "AND", output=True))
	print("=" * 50)
	print("Task 4. Quering with pagination")
	query_pagination(limit=3, offset=1)
	query_pagination(limit=1)
	print("=" * 50)
	print("Task 5. Universal deleting")
